...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		// a certain body defines a piece
		this.body = points;
		Arrays.sort(this.body);
		
		// initial width and height
		for(int i=0;i<this.body.length;i++) {
			this.width = Math.max(points[i].x+1, this.width);
			this.height = Math.max(points[i].y+1, this.height);
		}
	...
	*/
	public boolean equals(Object obj) {
		...
		Arrays.sort(other.body);
		if(Arrays.equals(other.body, this.body)) {
			return true;
		}
		return false;
...
//	}
	...
	private static Piece makeFastRotations(Piece root) {
		// YOUR CODE HERE
        root.next = helperMakeFastRotations(root, root);

		return root; 
	}

    private static Piece helperMakeFastRotations(Piece root, Piece nextRotation){
        Piece currPosition = nextRotation.computeNextRotation();
        if (currPosition.equals(root)) {
            return root;
        }else {
            currPosition.next = helperMakeFastRotations(root, currPosition);
            return currPosition;
        }
    }
